...
export async function getAvailableStorages() {
  const url = `${getConfig().CREDENTIALS_BASE_URL}/verifiable_credentials/api/v1/storages/`;
  let data = {};
  try {
    ({ data } = await getAuthenticatedHttpClient().get(url, { withCredentials: true }));
  } catch (error) {
...
export async function initVerifiableCredentialIssuance({ uuid, storageId }) {
  ...
  try {
    ...
    // We are catching and suppressing errors here on purpose. If an error occurs during the
    // initVerifiableCredentialIssuance call we will pass back an empty `data` object. Downstream we make
    // the assumption that if the initVerifiableCredentialIssuance object is empty that there was an issue or
    // error communicating with the service/API.
  }
  return data;
}
